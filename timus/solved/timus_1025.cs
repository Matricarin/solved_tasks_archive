...
namespace Timus
{
    public class Program
    {
        ...
        {
            var groupsAmount = int.Parse(Console.ReadLine());

            var groups = Console.ReadLine().Split(' ').Select(int.Parse);

            var ordered = groups.OrderBy(g => g).ToList();

            var min = ordered.Take(GetClosestToHalf(groupsAmount)).Select(n => GetClosestToHalf(n)).Sum();

            Console.WriteLine(min);
        }

        public static int GetClosestToHalf(int number)
        {
            return number / 2 + 1;
        }
    }
